...
bool wxMatRenParamsDlg::TransferDataToWindow()
{
	...
	m_matRenParams->LoadTextFile((std::tstring)GetMatRenParamsPath().c_str());
...
bool wxMatRenParamsDlg::TransferDataFromWindow()
{
	...
	m_matRenParams->SaveTextFile((std::tstring)GetMatRenParamsPath().c_str());
	return true;
}
...
void wxMatRenParamsDlg::OnLoadMaterialFilter( wxCommandEvent& event )
{
	...
	m_matRenParams->LoadMaterialFilter((std::tstring)matLibIniPath.c_str());
	UpdateListKnownMaterials();
}
